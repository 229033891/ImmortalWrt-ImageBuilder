...
CUSTOM_PACKAGES="$CUSTOM_PACKAGES bind-dig"
# 分区扩容（eMMC overlay 紧张时可用）
...
# luci-app-tailscale-community 已在上方 EasePi R1 段启用
# 内网穿透
...
# ddns-go 已在上方 EasePi R1 段启用（二进制）；若仓库有 luci-i18n-ddns-go-zh-cn 可再打开
